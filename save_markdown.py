...
def save_posts_to_markdown(posts_data, markdown_dir, image_dir):
    ...
    for item in posts_data:
        ...
        if post.get("type") == "image" and post.get("image_filename"):
            image_abs_path = os.path.abspath(os.path.join(image_dir, post["image_filename"]))
            md_lines.append(f"\n![]({image_abs_path})")
        md_lines.append("\n## Comments\n")
        if comments:
            for c in comments:
...
if __name__ == "__main__":
    ...
    Accepts --json argument for JSON file path (default: latest in data/post_json),
    then saves Markdown files to data/post_markdown with absolute image paths.
    """
    import argparse
    current_dir = os.path.dirname(os.path.abspath(__file__))
    json_dir = os.path.join(current_dir, "data", "post_json")
    markdown_dir = os.path.join(current_dir, "data", "post_markdown")
    ...
    parser.add_argument('--json', type=str, default=default_json, help=f"Path to JSON file (default: {default_json})")
    args = parser.parse_args()
    
    json_path = os.path.join(json_dir, args.json)
    markdown_dir = os.path.join(markdown_dir, ".".join(args.json.split(".")[0:-1]))
